...
import { ItemType, parseCropId, type IDayHarvest } from '~/assets/scripts/garden-planner/utils/garden-helpers';
    ...
    <section class="text-sm flex flex-col p-0 gap-1 dark:text-accent">
        <nav class="grid grid-cols-3 gap-1 pb-1">
            ...
            <div class="flex justify-center items-center gap-2">
                <button class="btn btn-xs btn-circle btn-ghost" aria-label="Previous Day" :disabled="!prevAvailable"
                    @click="$emit('prev')"><font-awesome-icon :icon="['fas', 'chevron-left']" class="" /></button>
                <h3 class="font-bold text-center">Day {{ day }}</h3>
                <button class="btn btn-xs btn-circle btn-ghost" :disabled="!nextAvailable" @click="$emit('next')"
                    aria-label="Previous Day"><font-awesome-icon :icon="['fas', 'chevron-right']" class="" /></button>
            </div>
            <div class="bg-primary invisible"></div>
        </nav>
        <div class="bg-secondary dark:bg-palia-blue-dark p-2 rounded-md" v-if="activeTab === 'default'">
            <section>
                ...
                <ul class="flex w-full overflow-x-auto gap-1 p-1 scrollbar-h-2 max-w-118 bg-opacity-50 rounded-md bg-misc-dark/20  dark:bg-palia-blue-dark join-item"
                    :class="[plannerDisplayConfig.get === 'display+display' ? 'xl:max-w-152 ' : 'xl:max-w-180']">
                    <template v-for="item of items" :key="item.name">
                        <ItemDisplayAlt class="border-misc border" v-if="item.count !== 0" :img-src="item.image"
            ...
            <section class="pt-2">
                <div class="flex gap-2 justify-between items-center pb-1">
                    ...
                    <button aria-label="See More" @click="activeTab = 'cropProcessTimes'"
                        class="btn btn-xs w-fit"><font-awesome-icon :icon="['fas', 'eye']" />View by crop</button>
                </div>
                ...
                <div class="grid grid-cols-3 gap-1">
                    <div class="flex gap-1 p-2 bg-misc/20 dark:bg-palia-blue rounded-xs justify-between flex-col">
                        ...
                        <p class="font-bold text-base gap-0.5"> <img width="16" height="16"
                ...
                <div class="grid grid-cols-2 gap-2 pt-1">
                    <div class="flex flex-col w-full">
                        <div class="flex items-end">
                            <img src="https://pgp-cdn.b-cdn.net/seeder.webp" class="max-w-5" alt="Seed Collectors"
                                aria-hidden="true">
                    ...
                    <div class="flex flex-col w-full">
                        <div class="flex items-end">
                            <img src="https://pgp-cdn.b-cdn.net/preserve-jar.webp" class="max-w-5" alt="Preserve Jars"
                                aria-hidden="true">
                            <p class="px-1 text-xs font-semibold text-palia-blue-dark dark:text-accent">
                                Preserve Jars
        ...
        <div class="flex flex-col gap-1" v-if="activeTab === 'cropProcessTimes'">
                ...
                <table class="table table-sm table-pin-rows ">
                    <thead class=" text-accent">
                        ...
                        <tr class="dark:bg-palia-blue-dark text-xxs h-fit">
                            <th class="py-0"></th>
                            <th class="py-0 whitespace-break-spaces"></th>
                            <th class="py-0 pb-0.5 whitespace-break-spaces">No Idle Time</th>
                            <th class="py-0 whitespace-break-spaces"></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr class=" not-last:border-b border-misc dark:border-water-retain [&>td]:nth-[2n]:bg-secondary [&>td]:nth-[2n]:dark:bg-palia-blue"
                            v-for="[cropId, processingInfo] in itemsProcessed.output.detailedProcessingInfo">

                            <td class="capitalize">
                                <p>
                                    <font-awesome-icon v-if="parseCropId(cropId).isStar" :icon="['fas', 'star']"
                            ...
                            <td class="">
                                <p>
                                    <img width="16" height="16" src="https://pgp-cdn.b-cdn.net/gold.webp"
                            ...
                            <td class="">
                                <p>
                                    ...
                                    <span class="text-xxs" v-if="craftersUsed.byCropId.get(cropId)">({{
