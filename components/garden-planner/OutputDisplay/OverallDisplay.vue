...
class="pr-1" :class="harvester.settings.useStarSeeds ? '' : ' opacity-50'"
    ...
    <section class="grid gap-1 md:grid-cols-2">
      <div v-if="processor.seedCollectorsCount > 0" class="flex flex-col w-full">
        ...
        <ul
          ...
          <ItemDisplay
v-for="[name, item] in processor.seedCollectors" :key="name" :img-src="item.img.src"
            :img-alt="item.img.alt" :star="item.isStar" :count="item.count" :base-gold-value="item.baseGoldValue"
            tooltip="Seed Collectors to use" />
        </ul>
      </div>
      <div v-if="processor.preserveJarsCount > 0" class="flex flex-col w-full">
        <div class="flex items-end">
        ...
        <ul
          ...
          <ItemDisplay
v-for="[name, item] in processor.preserveJars" :key="name" :img-src="item.img.src"
            :img-alt="item.img.alt" :star="item.isStar" :count="item.count" :base-gold-value="item.baseGoldValue"
